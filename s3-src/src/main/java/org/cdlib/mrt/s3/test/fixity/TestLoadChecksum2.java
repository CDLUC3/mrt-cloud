...
import org.cdlib.mrt.s3v2.tools.CloudChecksumV2;
/**
 *
 * @author replic
 */
public class TestLoadChecksum2 {
    ...
    public TestLoadChecksum2(LoggerInf logger) 
            throws TException
    {
        this.logger = logger;
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            TestLoadChecksum2 tc = new TestLoadChecksum2(logger);
            if (false) tc.dumpMulti(10,  9502, keySmall, 
            ...
            if (false) tc.dumpMulti(10,  9502, keyBig, digestBig, lenBig);
            
            if (false) tc.dumpMulti(10,  2002, keyDbl, digestDbl, lenDbl);
            
            if (true) tc.dumpMulti(10,  2002, keyDbl, digestDbl, lenDbl);
            
            if (false) tc.testValid(1, 9502, keySmall, 
                    "636c68fdf4ad96eac9e87adc478aeecbf6d867eaa3eeec3f6e98e4faf32d8e3a",  // right
                    //"636c68fdf4ad96eac9e87adc478aeecbf6d867eaa3eeec3f6e98e4faf32d8e3b", 
                    4412);
            if (false) tc.testValid(2, 9502, keySmall, 
                    "636c68fdf4ad96eac9e87adc478aeecbf6d867eaa3eeec3f6e98e4faf32d8e3a",  // right
                    //"636c68fdf4ad96eac9e87adc478aeecbf6d867eaa3eeec3f6e98e4faf32d8e3b", 
                    4412);
        ...
        throws TException
    {
        try {
            ...
            CloudStoreInf service = accessNode.service;
            String bucket = accessNode.container;
            CloudChecksumV2 cloudChecksum = CloudChecksumV2.getChecksums(types, service, bucket, key);
            System.out.println("begin:"
                    + " - metaObjectSize=" + cloudChecksum.getMetaObjectSize()
                    + " - metaSha256=" + cloudChecksum.getMetaSha256()
            );
            ...
            CloudChecksumV2.Digest test = cloudChecksum.getDigest(testType);
            System.out.println(test.dump("test digest"));
            
            CloudChecksumV2.CloudChecksumResult result = cloudChecksum.validateSizeChecksum(testChecksum, testType, testLen, logger);
